...
src/lib/api-key-auth.ts          — authenticateApiKey()（公開 API 金鑰驗證）/ generateApiKey()
                                   格式：jk_live_ + 16-byte hex，驗證後 fire-and-forget 更新 last_used_at
```
...
src/components/dashboard/DashboardSidebar.tsx — 響應式側邊欄（Client Component）
...
supabase/branches.sql                    ✅ （v0.14.0，多門市管理 + point_transactions.branch_id）
supabase/point-qrcodes.sql               ✅ （v0.14.1，QR Code 自助集點 + 兌換紀錄防重複）
supabase/line-messages.sql               ✅ （v0.14.2，LINE 訊息收件匣 + 方向索引）
...
/api/cron/auto-tag                  30 2 * * *   — 每日 02:30 UTC，自動標籤規則執行（全 tenant）
```
...
## 已驗證可用的功能（2026-04-24，v0.17.0）

**Dashboard（60 個頁面以上，全部已驗證）：**
- ✅ 登入、品牌設定、等級設定 CRUD
- ✅ 優惠券 CRUD、任務 CRUD、蓋章卡 CRUD、問卷 CRUD
- ✅ 掃碼集點（NT$500 × 3x = 1,500pt 驗證）、手動調整點數、**門市選擇**
- ✅ 會員管理（列表/搜尋/詳情/CSV 匯入匯出，CSV 含自訂欄位 + LINE UID）、備註 CRUD
- ✅ **會員詳情頁（/members/[id]）：完整資料 + 90 天活動時間軸 + 標籤管理 + 直接推播 + 備註**
- ✅ 點數記錄（搜尋/篩選/分頁/門市篩選）、Audit Log 寫入 + 讀取
- ✅ 分群 CRUD + 預覽、推播（立即/排程）、活動管理
- ✅ 抽獎、積分商城、標籤、加倍點數 CRUD
- ✅ 自動回覆規則、Webhook 建立 + 實際觸發 + 自動重試 + **測試按鈕**
- ✅ 自訂欄位定義 + 值 upsert、公告管理
- ✅ 生日獎勵、沉睡會員、黑名單、Rich Menu + 等級自動切換
- ✅ **即將生日會員頁（/members/birthdays，7/14/30/60 天篩選 + 等級過濾）**
- ✅ 數據總覽、RFM 分析、推播成效分析、數據報表（含 Cohort Retention）+ **Excel 匯出**
- ✅ **優惠券分析（發放量/核銷率/類型比較）**
- ✅ **任務完成分析（完成次數/參與率/發放點數）**
- ✅ **蓋章卡分析（參與人次/完成率/平均集章數）**
- ✅ 打卡集點 + 連續打卡獎勵
- ✅ 推播觸發規則（5 種類型）
- ✅ **推播訊息範本 CRUD（含變數提示：{{name}}/{{points}}/{{tier}}）**
- ✅ 問卷 CRUD + **問卷結果分析頁（題目統計圓餅/橫條 + 原始回覆表格）**
- ✅ 團隊管理（owner/staff 角色）
- ✅ 產業範本系統（Super Admin + 商家切換 + Setup Tasks）
- ✅ Production cron（birthday/expire-points/webhook-retry 已驗證）
- ✅ **自動標籤規則排程（每日 02:30 UTC 自動執行所有 tenant 的規則）**
- ✅ **門市管理（branches CRUD，掃碼集點/點數記錄整合）**
- ✅ **設定精靈（Setup Wizard + Banner + Overview 導引）**
- ✅ **QR Code 自助集點（CRUD + QR 圖下載 + LIFF 掃碼兌換 + 防重複）**
- ✅ **LINE 訊息收件匣（Webhook 儲存進出站訊息 + Dashboard 收件匣 UI）**
- ✅ **自動標籤規則（條件式自動套用標籤，手動觸發或逐條執行）**
- ✅ **會員排行榜（點數/消費/推薦三榜，Top 3 獎牌）**
- ✅ **門市業績分析（每門市交易/點數/人次/趨勢 sparkline）**
- ✅ **批量會員操作（checkbox 多選 → 批量貼標籤/推播/匯出選取，綠色工具列）**
- ✅ **API 金鑰管理（jk_live_ 前綴，Owner only，含 POS 整合文件）**
- ✅ **公開 API（/api/public/members 查詢、/api/public/points 新增點數，API 金鑰認證）**
- ✅ **手機響應式側邊欄（漢堡選單 + 滑入 Drawer，DashboardSidebar 客戶端元件）**
- ✅ **員工操作分析（/analytics/staff，依操作人統計後台動作，7/14/30/90 天）**
- ✅ **操作記錄 CSV 匯出（/api/audit-logs?export=csv，最多 5000 筆）**
...
- 15 個頁面：register / member-card / points / coupons / stamps / missions / store / **store/history** / referral / profile / surveys / checkin / my-brands / scan-qr / **announcements**
- 積分商城兌換成功後會送 LINE push 通知（after() 模式）
- member-card 公告區最多顯示 2 則，超過顯示「查看全部 →」連結

### Owner-only API（requireOwnerAuth）完整清單
以下 API 僅 `role='owner'` 可操作，staff 收到 403：
- `webhooks`、`campaigns`、`point-multipliers`、`audit-logs`（GET/export）、`rich-menu`
- `lotteries`、`reward-items`（積分商城）、`custom-fields`、`tier-settings`（POST/PATCH/DELETE）
- `tenants`（PATCH）、`checkin-settings`（PATCH）、`team`（全部）
