...
- ✅ 手動調整點數（補點 +100、扣點 -50）
- ✅ 會員管理（列表、搜尋、詳情）、會員備註 POST/GET
- ✅ 點數紀錄頁 `/dashboard/transactions`（搜尋/篩選/分頁）
- ✅ **Audit log 寫入**（v0.8.0 補齊 40 個 Dashboard mutation API）
- ✅ **Webhook 外送**（v0.10.0 補上呼叫 + after() 修復；points.earned delivery 已驗證建立）
- ✅ 分群 CRUD + 預覽、推播（立即/排程）、活動管理（批次給點/發券）
- ✅ CSV 匯入（含 `import_<UUID>` placeholder 修復）/匯出（含 tier display name）
- ✅ 自訂欄位定義 + 值 upsert（GET/POST /api/custom-field-values）
- ✅ 公告管理 CRUD（`/dashboard/announcements`）
- ✅ Production cron 驗證（birthday/expire-points，以正確 CRON_SECRET curl 通過）
- ✅ Tier 顯示名稱正確（blacklist/dormant-members/coupons/scan/segments/analytics 全修復）
- ✅ Model C Phase 3 — backfill 邏輯驗證通過

---

_最後更新：2026-04-22（v0.10.0）_
