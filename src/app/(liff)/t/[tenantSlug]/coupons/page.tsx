...
function formatCouponValue(coupon: Coupon): string {
  switch (coupon.type) {
    case 'discount': return `NT$${coupon.value} 折扣`
    case 'free_item': return '免費商品兌換'
    case 'points_exchange': return `兌換 ${coupon.value} 點`
...
export default function CouponsPage() {
  ...
  async function handleRedeem(memberCouponId: string) {
    ...
    try {
      ...
      setCoupons((prev) =>
        prev.map((mc) => mc.id === memberCouponId
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="px-4 mt-4">
        {coupons.length === 0 ? (
          <div className="rounded-2xl bg-white p-10 text-center text-gray-400 shadow-sm">目前沒有優惠券</div>
        ) : (
          <ul className="flex flex-col gap-3">
            {coupons.map((mc) => (
              <li key={mc.id} className="rounded-2xl bg-white px-5 py-4 shadow-sm flex flex-col gap-2">
                ...
                {mc.status === 'active' && (
                  <button onClick={() => handleRedeem(mc.id)} disabled={redeeming === mc.id}
