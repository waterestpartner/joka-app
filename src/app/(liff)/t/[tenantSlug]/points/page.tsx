...
import { useRealtimeMember, useRealtimePointTransactions } from '@/hooks/useRealtimeMember'
...
export default function PointsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchPoints() {
      try {
        const res = await fetch(`/api/points?tenantSlug=${tenantSlug}`, {
  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-10">
      <div className="bg-green-500 px-6 pt-10 pb-8 text-white text-center">
        <p className="text-sm font-medium text-green-100 uppercase tracking-widest mb-1">目前點數</p>
      ...
      <div className="px-4 mt-6">
        <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">點數記錄</h2>
        {transactions.length === 0 ? (
          <div className="rounded-2xl bg-white p-10 text-center text-gray-400 shadow-sm">尚無點數記錄</div>
        ) : (
          <ul className="flex flex-col gap-2">
            {transactions.map((tx) => {
              ...
              return (
                <li key={tx.id} className="flex items-center justify-between rounded-xl bg-white px-4 py-3 shadow-sm">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-medium text-gray-800">{TYPE_LABEL[tx.type]}</span>
                  ...
                  <span className={`text-lg font-bold ${amountColor}`}>
                    {sign}{formatNumber(Math.abs(tx.amount))} pt
                  </span>
                </li>
              )
            })}
