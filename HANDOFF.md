...
## 這個 session 完成了什麼（v0.12.2 + v0.12.3）
...
### v0.12.2 後續追加（同一 session）

| 修改 | 說明 |
|------|------|
| `supabase/liff-provider-type.sql` | 新增 `tenants.liff_provider_type`（enum）+ `tenants.line_login_channel_id`，備 LINE MINI App 轉換。⚠️ **需在 Supabase Console 執行 SQL** |
| `src/types/tenant.ts` | 補上 `platform_participation`、`liff_provider_type`、`line_login_channel_id` 三個缺漏欄位 |
| `src/types/member.ts` | 補上 `referral_code`、`platform_member_id`、`is_blocked`、`blocked_reason`、`blocked_at`、`last_activity_at`、`notes` 等 7 個缺漏欄位（optional） |
| `src/app/api/tenants/route.ts` | PATCH whitelist 加入 `liff_provider_type`、`line_login_channel_id` |
| LIFF 13 頁面程式碼審查 | 全部通過，沒有真實 bug（`!res.ok` 均已處理，`setLoading` 均在 finally） |

### v0.12.3 — Serverless 可靠性全面補強（本次 session 追加）

| 修改 | 說明 |
|------|------|
| `src/lib/line-auth.ts` | `verifyLineIdToken`（`/oauth2/v2.1/verify`）+ `verifyLineAccessToken`（`/v2/profile`）兩個 LINE API fetch 補上 `signal: AbortSignal.timeout(5000)` |
| `src/app/api/cron/scheduled-push/route.ts` | 每封 push 的 LINE API fetch 補上 `signal: AbortSignal.timeout(8000)` |
| `src/app/api/line-webhook/[tenantSlug]/route.ts` | `handleFollow()` + `handleMessage()` 從 `.catch()` fire-and-forget 改為 `after(() => handler(...).catch(...))` —— Vercel serverless 回傳 200 後，after() 保證工作完成再釋放，舊寫法會被提前 kill |
...
### 需要用戶在 Supabase Console 執行的 SQL

```
supabase/liff-provider-type.sql   ← 新增 liff_provider_type + line_login_channel_id 欄位
```
...
- 已用 `after()` 的地方：所有 `logAudit()`、所有 `fireWebhooks()`、所有 LINE push、LINE webhook `handleFollow()`/`handleMessage()`、`processReferral()`
...
```
src/lib/line-auth.ts                                 ← 兩個 LINE API fetch 加 AbortSignal.timeout(5000)
src/app/api/cron/scheduled-push/route.ts             ← LINE push fetch 加 AbortSignal.timeout(8000)
src/app/api/line-webhook/[tenantSlug]/route.ts       ← handleFollow/handleMessage 改用 after()
HANDOFF.md / TODO.md                                 ← 本次更新
```
...
```
(v0.12.3) fix(reliability): wrap handleFollow/handleMessage in after() in line-webhook ← 剛完成
2b02af0 fix(perf): add AbortSignal.timeout to remaining LINE API fetch calls          ← v0.12.3
2e4e01e fix(types): add missing optional fields to Member interface                   ← v0.12.2
004b31b fix(types): add missing platform_participation field to Tenant type           ← v0.12.2
e1d8df1 feat(schema): add liff_provider_type and line_login_channel_id to tenants    ← v0.12.2
fda86a3 docs: update HANDOFF.md and TODO.md to v0.12.2
2438b4a fix(members): use after() for referral reward to survive serverless lifecycle ← v0.12.2
977e577 feat(members): bind LINE UID to CSV-imported members on LIFF register         ← v0.12.2
9a05de9 feat(dashboard): wire SetupCard to setup wizard instead of settings page     ← v0.12.2
36c93a6 fix(security): sanitize ilike search in dormant-members and transactions     ← v0.12.1
```

> ✅ 所有 commit 已 push 到 origin/main（含 v0.12.3 最新修改）。
